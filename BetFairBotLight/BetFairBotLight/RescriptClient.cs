...
namespace BetFairBot
{
    public class RescriptClient : HttpWebClientProtocol, IClient
    {
        ...
        public T Invoke<T>(string method, IDictionary<string, object> args = null)
        {
            ...
            using (StreamReader reader = new StreamReader(stream, Encoding.UTF8))
            {
                ...
                if (response.StatusCode != HttpStatusCode.OK) {
                    throw ReconstituteException(JsonConvert.Deserialize<BetFairBot.TO.Exception>(jsonResponse));
                }
        ...
        private static System.Exception ReconstituteException(BetFairBot.TO.Exception ex)
        {
            var data = ex.Detail;
