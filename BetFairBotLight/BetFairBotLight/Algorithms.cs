...
namespace BetFairBot
{
    public partial class MainWindow
    {
        ...
        public void BetOnAllRecentlyFoundIncrementing(BettingLine betline)
        {
            ...
            betline.BetInstruction = new PlaceInstruction();
            betline.BetInstruction.LimitOrder = new LimitOrder();
            betline.BetInstruction.LimitOrder.PersistenceType = PersistenceType.PERSIST;
            betline.BetInstruction.OrderType = OrderType.LIMIT;
            betline.BetInstruction.Side = betline.betType == "Back" ? Side.BACK : Side.LAY;
    ...
    public partial class BettingLine
    {
                    ...
                    if (this.isActive)
                    {
                        try
                        {
                            ...
                            if (this.Filters.maxBetsCount > 0 ? (this.lastPlacedBetId.Trim(';').Split(';').Count() < this.Filters.maxBetsCount) : true)
                            {
                                ...
                                priceProjection.PriceData = new HashSet<PriceData>() { PriceData.EX_BEST_OFFERS };
                                priceProjection.RolloverStakes = true;
                                var language = "bg";
                                CurrentMarketBookList = MainWindow.Client.listMarketBook(resultList,priceProjection,OrderProjection.ALL,MatchProjection.ROLLED_UP_BY_PRICE,null,language).ToList();
        ...
        private bool IsMarketCompatible(MarketCatalogue market)
        {
        ...
        private void LastBetResult()
        {
            try 
	        {
                ...
                var settledOrdersList = MainWindow.Client.listClearedOrders(BetStatus.SETTLED, null, null, null, null, betIdsSet, null, null, null, true, null, null, null);
                ...
                var canceledOrdersList = MainWindow.Client.listClearedOrders(BetStatus.CANCELLED, null, null, null, null, betIdsSet, null, null, null, true, null, null, null);
                ...
                var voidedOrdersList = MainWindow.Client.listClearedOrders(BetStatus.VOIDED, null, null, null, null, betIdsSet, null, null, null, true, null, null, null);
                foreach (var voided in voidedOrdersList.ClearedOrders)
	            {
                    VoidedOrLapsedResult(voided);
	            }
                var lapsedOrdersList = MainWindow.Client.listClearedOrders(BetStatus.LAPSED, null, null, null, null, betIdsSet, null, null, null, true, null, null, null);
        ...
        private void VoidedOrLapsedResult(ClearedOrderSummary voided)
        {
            var context = new Entities();
            var yesterday = DateTime.Now.AddDays(-2);
        ...
        public static void UpdatePlacedBetResult(PlacedBets bet, string scoresLast2Days)
        {
            ...
            if (matchesLive.Length > 0 & matchNameCompoundsList.Count > 0)
            {
                foreach (var match in matchesLive)
                {
                    ...
                    matchNameCompoundsList.AddRange(bet.Selection.Split());
                    foreach (var item in matchNameCompoundsList)
                    {
                        matchNameCharNumber += item.Length;
                        if (match.Contains(item))
                    ...
                    compared = Math.Round((machingChars / matchNameCharNumber), 1);
                    if ((double)compared > 0.7)
                    {
                        var matchArray = match.Trim().Split().ToList();
                        var progress = matchArray.Last();
        ...
        private bool CancelBetAndResetBetId(CurrentOrderSummary order)
        {            
            CancelInstruction ci = new CancelInstruction();
            ci.BetId = order.BetId;
            var cireport = MainWindow.Client.cancelOrders(order.MarketId, new List<CancelInstruction>() { ci}, "");            
            if (cireport.ErrorCode == ExecutionReportErrorCode.OK & (cireport.Status == ExecutionReportStatus.SUCCESS | cireport.Status == ExecutionReportStatus.TIMEOUT))
            {
                return true;
            }
            return false;
        ...
        private void FindAndBetOnSutableRunner(MarketBook m)
        {
            ...
            foreach (var runner in sortedRunnerInfoByTotalAmmountMatched)
            {
                ...
                try
                {
                    if (BackAndLayPricesInRange(backPrices, layPrices))
                    {
                        ...
                        BetInstruction.Side = this.betType == "Back" ? Side.BACK : Side.LAY;
        ...
        private bool MarketIsPlayable()
        {
            try
            {
                ...
                var placedBets = from placedBet in context.PlacedBets
                                    ...
                                    select placedBet;
                if (!(placedBets.Count() > 0) & LiveScoreAvailable(CurrentMarket as MarketCatalogue))
                {
                    return true;
                }
                else
        ...
        private bool LiveScoreAvailable(MarketCatalogue eventName)
        {
            try
            {
                // \Soccer\Czech Soccer\Czech U21 League\Fixtures 08 August   \AC Sparta Praha U21 v FK Teplice U21
                ...
                if (matchesLive.Length > 0 & matchNameCompoundsList.Count > 0)
                {
                    foreach (var match in matchesLive)
                    {
                        ...
                        if ((double)compared > 0.7)
                        {
                            eventName.Event.Name = eventName.Runners[0].RunnerName + " v " + eventName.Runners[1].RunnerName;
                            if (this.Filters.goalDifferenceMin == null & this.Filters.goalDifferenceMax == null)
                            {
                                return true;
                            }
        ...
        private bool PlaceBet()
        {
            ...
            try
            {
                List<PlaceInstruction> pbinstructions = new List<PlaceInstruction>();
                ...
                if (placeExecutionReport.ErrorCode == ExecutionReportErrorCode.OK)
                {
        ...
        private PlacedBets SaveLastPlacedBet(PlaceInstructionReport placeBetsResult)
        {
            try
            {
                if (placeBetsResult.ErrorCode == InstructionReportErrorCode.BET_IN_PROGRESS | placeBetsResult.ErrorCode == InstructionReportErrorCode.BET_TAKEN_OR_LAPSED | placeBetsResult.ErrorCode == InstructionReportErrorCode.OK)
                {
                    var context = MainWindow.databaseContext;
                    var runner = CurrentMarket.Runners.FirstOrDefault(r => r.SelectionId == placeBetsResult.Instruction.SelectionId);
                    PlacedBets bet = new PlacedBets() { averagePrice = placeBetsResult.AveragePriceMatched, betId = placeBetsResult.BetId, betlineId = this.BetlineId, betType = this.betType, datePlaced = DateTime.Now, eventId = CurrentMarket.Event.Id, marketMenuPath = CurrentMarket.Event.Name, marketName = CurrentMarket.MarketName, resultCode = placeBetsResult.Status.ToString(), score = null, Selection = runner.RunnerName, sizeMatched = placeBetsResult.SizeMatched, sortedOrder = runner.SortPriority, success = false };
        ...
        private bool BackAndLayPricesInRange(List<PriceSize> backPrice, List<PriceSize> layPrice)
        {
        ...
        private bool IsSortedOrder(Runner runner, MarketCatalogue marketCatalogue, List<string> sortedOrderList)
        {
        ...
        {
            Entities context = MainWindow.databaseContext;
            context.Betlines.Add(MainWindow.CurrentBetline as Betlines);
            context.SaveChanges();
        }
    }
}
