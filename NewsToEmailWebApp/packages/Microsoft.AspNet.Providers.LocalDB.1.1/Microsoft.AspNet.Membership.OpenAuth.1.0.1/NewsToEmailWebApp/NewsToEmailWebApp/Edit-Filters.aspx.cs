    ...
    public partial class EditFilters : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                if (string.IsNullOrWhiteSpace(wsIdString))
                {
                    Server.Transfer("~/Account/Login.aspx");
                    return;
                }
        ...
        {
            var context = new Entities();
            CategoriesListView.DataSource = context.WebSites.FirstOrDefault(w => w.id == wsId).Categories.OrderByDescending(c => c.count).ToList();
        ...
        protected void AddFilter_Click(object sender, EventArgs e)
        {
            ...
            var fTextTB = (SiteMaster.FindControlRecursive(this, "FilterTextTB") as TextBox);
            var fText = fTextTB.Text;
            var fSizeTB = (SiteMaster.FindControlRecursive(this, "FilterSizeTB") as TextBox);
            var fSizeText = fSizeTB.Text;
            short? fSizeShort = null;
            if (!String.IsNullOrWhiteSpace(fSizeText))
            {
